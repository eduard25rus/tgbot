...
def normalize_expense_category_group_codes(group_codes: str | list[str] | tuple[str, ...] | set[str] | None) -> str:
    if isinstance(group_codes, str):
        raw_codes = re.split(r"[\s,;]+", group_codes)
    else:
        raw_codes = list(group_codes or [])
    cleaned: list[str] = []
    for code in raw_codes:
        normalized = str(code or "").strip()
        if normalized in EXPENSE_CATEGORY_GROUP_CODES and normalized not in cleaned:
            cleaned.append(normalized)
    return ",".join(cleaned or ["object"])
...
class Storage:
        ...
        with self.connection() as conn:
            conn.executescript(
                ...
                CREATE TABLE IF NOT EXISTS mobile_cash_access (
                    ...
                    design_version TEXT NOT NULL DEFAULT 'classic',
            ...
            if "design_version" not in mobile_cash_access_columns:
                conn.execute("ALTER TABLE mobile_cash_access ADD COLUMN design_version TEXT NOT NULL DEFAULT 'classic'")
            ...
            for code, group_codes in DEFAULT_EXPENSE_CATEGORY_GROUPS.items():
                conn.execute(
                    ...
                    SET group_codes = ?
                    WHERE code = ?
                      AND COALESCE(group_codes, '') = ''
                    """,
                    (group_codes, code),
                )
            for keyword in ("аренд", "связ", "коммун", "комисс", "налог", "пошлин", "штраф", "офис", "бухгалтер", "юрист", "подпис", "софт", "интернет"):
                conn.execute(
                    """
                    UPDATE expense_categories
                    SET group_codes = 'admin'
                    WHERE COALESCE(group_codes, '') = ''
                      AND LOWER(COALESCE(label, '')) LIKE ?
                    """,
                    (f"%{keyword}%",),
                )
            conn.execute(
                """
                UPDATE expense_categories
                SET group_codes = 'transfer'
                WHERE COALESCE(group_codes, '') = ''
                  AND LOWER(COALESCE(label, '')) LIKE '%касс%'
                  AND (
                    LOWER(COALESCE(label, '')) LIKE '%перев%'
                    OR LOWER(COALESCE(label, '')) LIKE '%вывод%'
                    OR LOWER(COALESCE(label, '')) LIKE '%перемещ%'
                  )
                """
            )
            conn.execute(
                """
                UPDATE expense_categories
                SET group_codes = 'income'
                WHERE COALESCE(group_codes, '') = ''
                  AND LOWER(COALESCE(label, '')) LIKE '%поступ%'
                """
            )
            conn.execute(
                """
                UPDATE expense_categories
                SET group_codes = 'object'
                WHERE COALESCE(group_codes, '') = ''
                """
            )
        ...
        with self.connection() as conn:
            ...
            rows = conn.execute(
                ...
                SELECT user_id, owner_chat_id, enabled, role, default_cashbox_code,
                       default_screen, design_version, allowed_cashbox_codes, preview_login, preview_password_hash,
    ...
    def update_mobile_cash_access(
        ...
        if default_screen not in {"home", "history", "letters", "work"}:
            default_screen = "home"
        if design_version not in {"classic", "v2"}:
            design_version = "classic"
        ...
        with self.connection() as conn:
            ...
            conn.execute(
                ...
                INSERT INTO mobile_cash_access (
                    user_id, owner_chat_id, enabled, role, default_cashbox_code, default_screen, design_version,
            ...
            for index, (code, label) in enumerate(DEFAULT_EXPENSE_CATEGORIES, start=1):
                ...
                if existing is None:
                    ...
                    conn.execute(
                        """
                        UPDATE expense_categories
                        SET group_codes = ?, updated_at = ?
                        WHERE id = ? AND owner_chat_id = ? AND COALESCE(group_codes, '') = ''
                        """,
                        (
                            normalize_expense_category_group_codes(DEFAULT_EXPENSE_CATEGORY_GROUPS.get(code, "object")),
                            now,
    ...
    def add_expense_category(
        ...
        if not cleaned_label:
            return None
        cleaned_groups = normalize_expense_category_group_codes(group_codes)
        now = datetime.utcnow().isoformat()
        with self.connection() as conn:
            existing = conn.execute(
                """
    ...
    def _mobile_cash_access_from_row(self, row: sqlite3.Row) -> dict:
        return {
            ...
            "design_version": row["design_version"] if "design_version" in row.keys() and row["design_version"] in {"classic", "v2"} else "classic",
