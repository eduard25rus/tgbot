...
class Storage:
    ...
    def delete_payroll_employee_with_legacy_payroll(self, owner_chat_id: int, employee_id: int, cutoff_month: date) -> dict[str, int]:
        cutoff = cutoff_month.replace(day=1)
        with self.connection() as conn:
            employee = conn.execute(
            ...
            if int(work_reports_count or 0) or int(direct_expenses_count or 0) or int(allocated_expenses_count or 0):
                raise ValueError("Нельзя удалить старые строки: у сотрудника есть смены или расходы ДДС")
            current_rows = conn.execute(
                ...
                """,
                (owner_chat_id, employee_id, cutoff.strftime(DATE_FMT)),
            ).fetchall()
            if current_rows:
                months = ", ".join(row["payroll_month"][:7] for row in current_rows[:6])
                raise ValueError(f"У сотрудника есть строки ФОТ начиная с {cutoff.strftime('%Y-%m')}: {months}")
            payroll_cursor = conn.execute(
                """
            ...
            if employee_cursor.rowcount <= 0:
                raise ValueError("Сотрудник не найден")
        return {
            "payroll": int(payroll_cursor.rowcount or 0),
            "rates": int(rate_cursor.rowcount or 0),
            "employees": int(employee_cursor.rowcount or 0),
        }
