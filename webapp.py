...
def render_cashoperations_body(
    ...
    income_screen = '<div class="cash-mobile-empty">Нет прав на добавление поступлений.</div>'
    if can_edit:
        income_screen = f"""
        <section class="cash-mobile-panel">
    ...
    <script>
      (() => {{
        ...
        function refreshIfStale(screenName, minAgeMs) {{
          if (Date.now() - loadedAt < minAgeMs) return false;
          return refreshCash(screenName);
        }}
            ...
            <article class="user-card">
              <form class="form-grid" method="post" action="/access/cash/users/{user["id"]}/update?owner={owner_chat_id}&mode=cash">
                <div class="user-head">
                  ...
                  <div class="badge-row">
                    <span class="badge{" danger" if not cash_enabled else ""}">{"Касса включена" if cash_enabled else "Касса отключена"}</span>
...
def render_expenses_section(
    ...
    day_cards_html = "".join(
        f"""
        <a class="expenses-day-card{' is-selected' if active_selected_day == day else ''}" href="{build_expenses_href(day=day)}" data-expenses-day-card="1">
    ...
    def render_cash_income_rows(row_entries) -> str:
        return "".join(render_cash_income_row(entry) for entry in row_entries)
    ...
    if path == "/cashoperations/income" and method == "POST":
        ...
        try:
            ...
            if expense_date is None:
                raise ValueError("Укажите дату поступления")
            amount = parse_amount(form.get("amount", "").strip())
            source = form.get("source", "other").strip()
            source_label = source_labels.get(source, "Другое")
